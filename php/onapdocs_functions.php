...
$file_name = $dir_path . "/" . "json/comp_images.json";
...
$dir_path= format_db_path();
$file_name = $dir_path . "/" . "css/menu_style.css";
...
{ 
	...
	// topics.json is a synbolic line to onap_releases.json which 
	// has the uri to the specific release main image.png file
	//

$dir_path= format_db_path();
$file_name = $dir_path . "/" . "json/topics.json";
...
echo "<!-- " . "These comments are generated by php/onapdocs_functions.php to help with debbuging  " .
" -->" . PHP_EOL;
echo "<!-- " . "Titles File is: [ "
        . $file_name .
" ] -->" . PHP_EOL;
//
// End of HTML comments generation
//
...
/*
 * format the dir path to the /data/navigator_db dirs
 */

function format_db_path()
{

	$json_db = "json_db";
	$navigator_db = "/data/navigator_db";
	$data_dir_path = $navigator_db . "/" .  $_SESSION["path"] . "/" . $json_db;
//echo " In format_db_path() - data dir_path: " . $data_dir_path . PHP_EOL;
...
// set_img_uri()
//
function set_img_uri($image_file)
{
	//
	// If image_uri in the json file is a link then do nothing.
	//  Otherwise, format the relative pathname to the image file 
	//  as follows:
	//  	common/images/"path-uri"/"filename form .json file"

	$common_img_dir = "common/images";
	$image_relative_path = $common_img_dir . "/" .  $_SESSION["path"] . "/" . $image_file;
//echo ">>>In set_img_uri() - image_relative_path: " . $image_relative_path . PHP_EOL;
...
//
//
function set_config_path ($base_dir_path, $topic_name, $config_file_name)
{

	//
	// if config_file exists in the local_congif dir use
	// it, otherwise look for default file  in json_db/global_config
	//

	$local_config_file = $topic_name . "/" . "local_config" . "/" . $config_file_name; 
	$global_config_file = "global_config" . "/" . $config_file_name; 

	$local_config_file_path =  $base_dir_path . "/"  . $local_config_file;  
//echo "In set_config_path(): local config: " . $local_config_file_path . PHP_EOL; 
	

	$global_config_file_path =  $base_dir_path . "/" . $global_config_file  ;

	if ( file_exists ($local_config_file_path) === TRUE) {

//echo "In set_config_path(): returning local config: " . $local_config_file_path . PHP_EOL; 
		return ($local_config_file_path);
	} else {
//echo "In set_config_path(): returning global: " . $global_config_file_path . PHP_EOL; 
...
//	get_json_attr()
//
function get_json_attr ($json_file_name_path, $json_attr)
{

//echo "in: " , __FILE__ . ", line " . __LINE__ . " (" . $json_file_name_path . ")";

if (file_exists ($json_file_name_path) === FALSE) {
	//
	// Return an error msg if the nav)config.json file does not exist
	//
$err_msg = <<<EOT
file $json_file_name_path does not exist.
EOT;
	return ($err_msg);
}
...
$data = json_decode($readjson, true);
	...
	foreach ($data as $rec) {
			//echo "attr: (" . $rec[$json_attr] . ")";
			return (  str_replace ("\r|\n", "", $rec[$json_attr]) );
	}
}

function prt_debug_as_comments ($dir_path, $source_file, $line_num)
{
	if (isset ($d) == FALSE) {
//		echo "\nd debug class does not exist, create it...\n";
		$d = new nav_debug();
	}
	else {

		echo '$d debug class exists';
	}

	//
	// Debug mode is either 'on' or 'off'
	//
	$d->debug_mode = $_SESSION["debug_mode"];
//echo "in ", __FILE__ . ", line ", __LINE__ . " - debug_mode: " . $d->debug_mode . PHP_EOL; 
	$d->dir_path  = $dir_path;
	$d->script_name  = $source_file .", line " . $line_num ;
	$d->prt_comments();
	//unset ($d);
